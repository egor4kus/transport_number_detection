...
Здесь лежит AI-слой проекта. Для учебного проекта структура сделана простой: есть три версии `baseline`, `mvp`, `target`, и для каждой версии предусмотрен свой Python-пайплайн.
...
Важно: `ai/` сам по себе не поднимает HTTP-сервер и не имеет собственного порта.
Во всех пользовательских сценариях доступ к AI-слою идет через `backend/`,
который импортирует `ai.service`.
